...
import { User, Mail, Phone, MapPin, Camera, Save, Activity, Heart, AlertCircle, CheckCircle2 } from 'lucide-react';
          ...
          <div className="bg-white dark:bg-slate-800 p-6 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm text-center">
            <div className="relative inline-block mb-4">
              <div className="w-24 h-24 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 flex items-center justify-center text-3xl font-bold border-4 border-white dark:border-slate-800 shadow-sm mx-auto">
                {demoPatient.name.split(' ').map((n: string) => n[0]).join('')}
              </div>
              <button className="absolute bottom-0 right-0 p-1.5 bg-blue-600 text-white rounded-full border-2 border-white dark:border-slate-800 hover:bg-blue-700 transition-colors">
                <Camera className="w-4 h-4" />
              </button>
            </div>
            <h2 className="font-bold text-slate-900 dark:text-white">{demoPatient.name}</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{demoPatient.id}</p>
